...
ChessResult chessSavePlayersLevels(ChessSystem chess)
{
    int n = mapGetSize(chess->players);
    int *levels_arr = malloc(n * sizeof(int));
    Player tmp_player = NULL;
    int i = 0;
    MAP_FOREACH(int *, player_id, chess->players)
    {
        tmp_player = mapGet(chess->players, player_id);
        levels_arr[i++] = getPlayerLevel(tmp_player);
        free(player_id);
    }

    bubbleSort(levels_arr, n);
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            tmp_player = mapGet(chess->players, player_id);
            if(levels_arr[i] == getPlayerLevel(tmp_player))
            {
                printf("id: %d level: %d",getPlayerId(tmp_player),levels_arr[i]);
                i++;
            }
            free(player_id);
        }
